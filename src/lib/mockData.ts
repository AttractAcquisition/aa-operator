...
export const mockSOPs: SOP[] = [
  ...
  { id: 'sop_02', num: '02', name: 'Prospect Scraper & Batch Run', domain: 'Distribution', tier: 'AUTO', cron_expression: '0 9 * * 1', schedule_label: 'Mon/Wed 09:00', tools: ['trigger_scraper_run', 'stage_batch_results'], model: 'claude-sonnet-4-6', is_active: true, last_run: new Date(Date.now() - 1000*60*60*72).toISOString(), run_count: 12, description: 'Triggers the prospect scraper for the selected source list and stages results for enrichment.' },
  { id: 'sop_03', num: '03', name: 'Prospect Enrichment, QA & Dedup', domain: 'Distribution', tier: 'AUTO', tools: ['web_search', 'add_enrichment_data', 'flag_duplicate'], model: 'claude-sonnet-4-6', is_active: true, last_run: new Date(Date.now() - 1000*60*60*72).toISOString(), run_count: 12, description: 'Researches each prospect online, scores quality 1-10, removes duplicates, stages qualifying leads.' },
  { id: 'sop_04', num: '04', name: 'Prospect Import & CRM Staging', domain: 'Distribution', tier: 'AUTO', cron_expression: '0 8 * * 1-5', schedule_label: 'Weekdays 08:00', tools: ['stage_leads_to_batch', 'update_prospect_status'], model: 'claude-haiku-4-5', is_active: true, last_run: new Date(Date.now() - 1000*60*160).toISOString(), run_count: 62, description: 'Moves clean enriched leads into the daily outreach batch in the correct priority order.' },
  ...
  { id: 'sop_06', num: '06', name: 'Reply Triage & CRM Hygiene', domain: 'Distribution', tier: 'AUTO', cron_expression: '30 8 * * *', schedule_label: 'Daily 08:30', tools: ['get_unread_replies', 'update_prospect_status', 'create_approval_item'], model: 'claude-sonnet-4-6', is_active: true, last_run: new Date(Date.now() - 1000*60*150).toISOString(), run_count: 84, description: 'Classifies all incoming WhatsApp replies. Warm leads queued for your action. CRM updated automatically.' },
  { id: 'sop_07', num: '07', name: 'Discovery Call Booking & Prep', domain: 'Distribution', tier: 'ASSISTED', tools: ['generate_call_brief', 'create_approval_item'], model: 'claude-opus-4-6', is_active: true, run_count: 28, description: 'Generates detailed call brief for warm leads — company research, pain points, suggested angles.' },
  { id: 'sop_08', num: '08', name: 'MJR Build', domain: 'Distribution', tier: 'AUTO', tools: ['get_prospect_detail', 'web_search', 'generate_mjr_report', 'store_document'], model: 'claude-opus-4-6', is_active: true, run_count: 34, description: 'Generates the full personalised Missed Jobs Report HTML document from prospect data and web research.' },
  ...
  { id: 'sop_12', num: '12', name: 'SPOA Build', domain: 'Distribution', tier: 'AUTO', tools: ['get_prospect_detail', 'web_search', 'generate_spoa_document', 'store_document'], model: 'claude-opus-4-6', is_active: true, run_count: 18, description: 'Builds the full Strategic Plan of Action document post-MJR call using prospect data and web research.' },
  ...
  { id: 'sop_47', num: '47', name: 'Weekly Client Reporting', domain: 'Delivery', tier: 'AUTO', cron_expression: '0 17 * * 5', schedule_label: 'Friday 17:00', tools: ['get_delivery_metrics', 'generate_weekly_client_report', 'publish_to_portal'], model: 'claude-opus-4-6', is_active: true, last_run: new Date(Date.now() - 1000*60*60*48).toISOString(), run_count: 12, description: 'Generates weekly performance reports for all active clients. Approval required before publishing to portal.' },
  { id: 'sop_56', num: '56', name: 'Finance Dashboard & Income Tracking', domain: 'Finance', tier: 'AUTO', cron_expression: '0 7 * * 1', schedule_label: 'Monday 07:00', tools: ['get_finance_data', 'generate_finance_summary'], model: 'claude-sonnet-4-6', is_active: true, last_run: new Date(Date.now() - 1000*60*60*72).toISOString(), run_count: 12, description: 'Aggregates weekly income and expense data, flags overdue invoices, updates finance dashboard.' },
